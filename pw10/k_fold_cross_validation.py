...
def compute_confusion_matrix(target, output, threshold):
    ...
    if output.shape[1] == 1:
        ...
        tuples = [[False] * n_classes for _ in output_maxs]
        for i, t in enumerate(tuples):
            t[output_maxs[i]] = True
...
def k_fold_cross_validation(mlp, dataset, k=5, learning_rate=0.01, momentum=0.7, epochs=100, threshold=None):
    ...
    if threshold is None:
        ...
        CM = compute_confusion_matrix(target_test, output_test, threshold)
        
        return (MSE_train_mean / k, MSE_test_mean / k, CM)
